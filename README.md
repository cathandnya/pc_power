...
| GPIO24 | 入力 | SPEAKER ヘッダー | 10kΩクランプ経由（5V/3.3V両対応） |
...
ユニバーサル基板上の分圧回路にZero Wの3.3V（Pin 1）から入力し、中点の電圧を測定する。

| 入力 | 測定箇所 | 期待値 |
|------|---------|--------|
| Pin 1 (3.3V) → 6.8kΩ入力側 | 3.3kΩ上端（GPIO接続点） | ≈1.09V |
| VBUS (5V) → 6.8kΩ入力側 | 3.3kΩ上端（GPIO接続点） | ≈1.65V |

#### 入力ピンの確認（Web UI反映）

Zero Wでアプリを起動した状態で、Pin 1（3.3V）からジャンパー線で各入力回路にテスト信号を入れる。

| テスト | 方法 | 期待値 |
|--------|------|--------|
| PWR_LED | 3.3V → 分圧回路入力 → GPIO22 | Web UI の電源アイコン点灯 |
| HDD_LED | 3.3V → 分圧回路入力 → GPIO23 | Web UI のディスクアイコン点灯 |
| SPEAKER | 3.3V → 10kΩ → GPIO24 | Web UI のビープアイコン点灯 |
