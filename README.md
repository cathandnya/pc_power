...
PC スピーカーは矩形波でコイルを駆動するタイプのため、ビープ中は GPIO24 も同じ周波数で HIGH ↔ LOW を繰り返す。本実装では 50ms (`MONITOR_INTERVAL`) ごとに GPIO の**瞬時値**をサンプリングしてアクティブ判定しているため、矩形波の HIGH 側に当たった瞬間だけ `beep=True` となる確率的な検出になる。

- **POST ビープ（100〜300ms 程度の「ピッ」）** はポーリング周期より十分長いので、ビープ中に何度かサンプリングが入り、そのうち 1 回でも HIGH を拾えれば検出できる
- **50ms より短いパルス音**は取りこぼす可能性がある
- **連続ビープ（数秒の「ピー」）と短いビープ（数百ms の「ピッ」）の区別はつかない**（Web UI には単に「鳴っている／いない」としか出ない）

POST 時の警告ビープを拾う用途には十分だが、短いパルスや音のパターンを精密に判別する用途には向かない。より厳密に検出したい場合は、ハードウェアで矩形波を整流・平滑して DC 化するか、GPIO エッジ検出でカウントする方式へ変更する必要がある（本 PR のスコープ外）。
