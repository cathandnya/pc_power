...
| フォトカプラ PC817 | 2 | PWR_SW / RST_SW の絶縁 |
| 抵抗 330Ω | 2 | PC817 LED 電流制限 |
...
|------|---------|------|--------|------|
| GPIO17 | 11 | 出力 | PC817 (PWR_SW用) Pin2 | 通常HIGH、操作時LOW。Pin1は330Ω経由で3.3V |
| GPIO27 | 13 | 出力 | PC817 (RST_SW用) Pin2 | 通常HIGH、操作時LOW。Pin1は330Ω経由で3.3V |
| 3.3V | 1 | - | PC817 (×2) Pin1 | 330Ω経由 |
| GND | 14 | - | Pi 側 GND | **MB GND とは分離** |
...
### PWR_SW / RST_SW の絶縁（PC817）

電源スイッチ・リセットスイッチは、Zero W 故障時にマザーボードを巻き添えにしないようフォトカプラ PC817 で電気的に絶縁する。Pi GND と MB GND は完全に分離する。

```
    [ Pi 側 (1次) ]          [ PC817 ]        [ MB 側 (2次) ]
...
PC817 の 2 次側（Pin4 ↔ Pin3）と物理ボタンが **PWR_SW+ と MB GND の間に並列**に入る。どちらが ON になっても PWR_SW+ が MB GND へ短絡されて電源スイッチが押された扱いになる。Pi 3.3V / Pi GND と MB GND は一切繋がっておらず、光結合のみで信号が渡る。

GPIO を `OUTPUT HIGH` で待機し、押下時のみ `OUTPUT LOW` にする。LOW のとき 3.3V → 330Ω → PC817 LED → GPIO の経路で電流が流れ、LED 点灯 → 2 次側が導通 → PWR_SW+ が MB GND へ短絡されて押下状態になる（GPIO=LOW=押下）。

### 分圧回路（PWR_LED / HDD_LED 共通）

マザーボードのLEDヘッダーは5V出力の場合があるため、分圧抵抗でGPIOを保護する。本構成では LED 入力・SPEAKER 入力側は Pi GND 共通（後述）としており、この部分は**絶縁されていない**。PWR_SW / RST_SW のみを絶縁する部分絶縁構成となる。
...
本構成は **部分絶縁** である。PWR_SW / RST_SW 経路（最も電流が大きく、故障時の被害が大きい）は PC817 で完全絶縁されるが、LED / SPEAKER の読み取りのために Pi GND とマザーボード GND は結果的に接続される。そのため外来サージや Pi 側電源故障の一部リスクは残る。日常運用での信号電流は微小で実害は出にくい一方、完全絶縁を求める場合は LED / SPEAKER 側も別途フォトカプラが必要になる。

GND は **2 系統に分けて配線する**:

**Pi GND**（Zero W 側）
- Zero W の GND ピン（Pin 6, 9, 14, 20, 25, 30, 34, 39 のいずれか）
- 分圧回路の 3.3kΩ 下端（PWR_LED / HDD_LED）
- SPEAKER の 10kΩ クランプの GND 側
- マザーボードの LED-, SPEAKER- ヘッダー（信号リターン）

**MB GND**（マザーボード側、基板上では Pi GND と直接接続しない）
- PC817 (×2) の Pin3（Emitter）
- 物理ボタン（PWR BTN / RST BTN）のGND側
- マザーボードのフロントパネル PWR_SW- / RST_SW- ヘッダー

PWR_LED-, HDD_LED-, SPEAKER- は Pi GND 共通、PWR_SW-, RST_SW- のみ MB GND 側という振り分けになる。ユニバーサル基板上で Pi GND レールと MB GND レールを物理的に分けて配線する。

> **注意**: 基板上では 2 つの GND レールを分けているが、LED- / SPEAKER- 経由でマザーボード内部では Pi GND と MB GND は結局繋がってしまう。よって本構成は「スイッチ経路の絶縁」にとどまる。完全絶縁を目指す場合は LED / SPEAKER 入力側にも別途フォトカプラを挟む必要がある。

## 配線

### 接続手順

1. **基板を製作**: ユニバーサル基板にピンヘッダーをはんだ付け（PWR_SW / RST_SW / HDD_LED 各2本、SPEAKER 2本）。Pi GND レールと MB GND レールを**分けて**配置
2. **PC817 を実装**: PWR_SW 用・RST_SW 用に PC817 を2個、それぞれに 330Ω を Pin1 側に実装
3. **分圧回路を実装**: PWR_LED用・HDD_LED用の分圧回路を基板上にはんだ付け（Pi GND 側）
4. **ケースのボタンを接続**: 電源ボタン・リセットボタンのコネクタを MB GND 側のピンヘッダーに差し込む
5. **マザーボードと接続**: デュポンケーブルで基板からマザーボードの各ヘッダーへ接続
6. **Zero W と接続**: デュポンケーブルで基板から Zero W の各GPIO / 3.3V / Pi GND へ接続
